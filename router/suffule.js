...
module.exports = (shuffule, jwt, knex) => {
    ...
    shuffule
        .post('/shuffule', async (req, res) => {
            ...
            // console.log(students_data.length, bed_Data.length)

            if (students_data.length <= bed_Data.length) {
        ...
        .get('/getShuffuleData', (req, res) => {
            ...
            knex.select('*').from('signup')
                ...
                .leftJoin('userPics', 'userPics.email', 'signup.email')
                .orderBy('firstName')
                // .where('signup.email','assign_beds.email')
                .then(data => {
